...
export const useTrainingCampProviders = () => {
    ...
    const fetchProviders = useCallback(async () => {
        setIsLoading(true);
        try {
            const fetchedProviders = await getAllTrainingCampProviders();
            setProviders(fetchedProviders);
        } catch (error) {
            console.error('Failed to fetch training camp providers:', error);
    ...
    const addProvider = async (newProviderData: Omit<Provider, 'id'>) => {
        try {
            await addTrainingCampProvider(newProviderData);

...
export default function AddClubPage() {
    ...
    useEffect(() => {
        const fetchConfig = async () => {
            ...
            if (config?.leagueStructures) {
                const countryList = Object.values(config.leagueStructures).map((structure: any) => ({
                    id: structure.id,
                    countryName: structure.countryName,
                }));
                            ...
                            <div className="space-y-2">
                             ...
                             <div className="space-y-2">
                                ...
                                <Select value={formData.country} onValueChange={(val) => setFormData(p => ({...p, country: val}))}>
                                    ...
                                    <SelectContent>
                                        {countries.map(c => <SelectItem key={c.id} value={c.id}>{c.countryName}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                            </div>
                        </div>

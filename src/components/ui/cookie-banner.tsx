...
export const CookieBanner = () => {
    const [settings, setSettings] = useState({
        ...
        icon: "cookie" as "cookie" | "shield" | "info",
        privacyHref: "/privacy",
        termsHref: "/terms",
    });
  ...
  useEffect(() => {
    const storedSettings = localStorage.getItem("amigoal_cookie_settings");
    if (storedSettings) {
        try {
  ...
  const IconEl =
    settings.icon === "shield" ? Shield : settings.icon === "info" ? Info : Cookie;
  ...
  return (
    <div
      ...
      className="fixed right-4 bottom-4 md:right-6 md:bottom-6 z-[150] w-[360px] max-w-[90vw]"
    >
      <div
        className={cn(
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-sm font-semibold leading-5">{settings.title}</h2>

...
export const FeedbackCard = () => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        'w-fit overflow-hidden border py-2 shadow-sm dark:border-neutral-800 bg-card'
      )}>
      <span className="flex items-center justify-center gap-3 pl-4 pr-2">
        <div className="text-sm text-card-foreground dark:text-neutral-400">Gefällt Ihnen Amigoal?</div>
        ...
        <AnimatePresence>
          {!isSubmitted ? (
            <motion.span exit={{ opacity: 0 }} initial={{ opacity: 1 }}>
              <textarea
                ref={textRef}
                placeholder="Ihre App ist grossartig!"
              ...
              <div className="flex h-fit w-full justify-end">
                <button
                  onClick={() => submitFeedback({happiness: happiness!, feedback: textRef.current!.value || ''})}
                  className={cn(
            ...
            <motion.div
              ...
              <motion.div variants={itemVariants}>Ihr Feedback wurde empfangen!</motion.div>
              <motion.div variants={itemVariants}>Vielen Dank für Ihre Hilfe.</motion.div>
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>

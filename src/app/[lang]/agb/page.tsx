...
const AgbSection = ({ title, children }: { title: string, children: React.ReactNode}) => (
     ...
     useEffect(() => {
        const fetchContent = async () => {
            ...
            try {
                ...
                setContent(translations.agb_page);
            } catch (error) {
    ...
    return (
        <div className="bg-muted/20 text-foreground min-h-screen flex flex-col">
            ...
            <main className="flex-grow container mx-auto max-w-4xl px-4 py-16 md:py-24">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="text-3xl font-bold font-headline">{pageContent.title || 'Allgemeine Geschäftsbedingungen'}</CardTitle>
                    ...
                    <CardContent>
                        {(pageContent.sections || []).map((section, index) => (
                             <AgbSection key={index} title={section.title}>
                                {section.content}
                            </AgbSection>
                        ))}
                    </CardContent>
